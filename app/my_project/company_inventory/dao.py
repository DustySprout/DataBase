...
class AccessPointsDAO:
    ...
    def get_access_point_by_id(access_point_id):
        try:
            cursor = current_app.mysql.connection.cursor()
            cursor.execute("SELECT * FROM access_points WHERE id = %s", (access_point_id,))
            row = cursor.fetchone()
        except Exception as e:
    ...
    def add_access_point(serial_number, model):
        try:
            cursor = current_app.mysql.connection.cursor()
            cursor.execute("INSERT INTO access_points (serial_number, model) VALUES (%s, %s)", (serial_number, model))
    ...
    def update_access_point(access_point_id, serial_number, model):
        try:
            cursor = current_app.mysql.connection.cursor()
            cursor.execute("UPDATE access_points SET serial_number = %s, model = %s WHERE id = %s", (serial_number, model, access_point_id))
            current_app.mysql.connection.commit()
        except Exception as e:
    ...
    def delete_access_point(access_point_id):
        try:
            cursor = current_app.mysql.connection.cursor()
            cursor.execute("DELETE FROM access_points WHERE id = %s", (access_point_id,))
            current_app.mysql.connection.commit()
        except Exception as e:
...
class SelectDAO:
    ...
    def get_all_employee_equipment():
        try:
            cursor = current_app.mysql.connection.cursor()
            cursor.execute("SELECT e.first_name, e.last_name, c.model AS computer_model, m.model AS monitor_model, p.model AS printer_model, r.model AS router_model, ap.model AS access_point_model, ip.model AS ip_phone_model FROM employee_equipment ee JOIN employees e ON ee.employee_id = e.id JOIN computers c ON ee.computer_id = c.id JOIN monitors m ON ee.monitor_id = m.id JOIN printers p ON ee.printer_id = p.id JOIN routers r ON ee.router_id = r.id JOIN access_points ap ON ee.access_point_id = ap.id JOIN ip_phones ip ON ee.ip_phone_id = ip.id ORDER BY e.last_name")
    ...
    def get_all_office_equipment():
        try:
            cursor = current_app.mysql.connection.cursor()
            cursor.execute("SELECT o.name AS office_name, p.model AS printer_model, r.model AS router_model, ap.model AS access_point_model FROM office_equipment oe JOIN offices o ON oe.office_id = o.id JOIN printers p ON oe.printer_id = p.id JOIN routers r ON oe.router_id = r.id JOIN access_points ap ON oe.access_point_id = ap.id ORDER BY o.name")
